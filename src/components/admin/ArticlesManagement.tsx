...
import { Plus, Pencil, Trash2, FileText, Clock, Eye, EyeOff, Newspaper } from 'lucide-react';
...
export const ArticlesManagement = () => {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      category: form.category, image_url: form.image_url || null, is_published: form.is_published,
    };
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <h3 className="font-bold text-foreground">{article.title_ar}</h3>
                    ...
                    {article.summary_ar && (
                      <p className="text-sm text-muted-foreground/80 line-clamp-2 mb-3">{article.summary_ar}</p>
                    )}
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-primary/10" onClick={() => openEdit(article)}>
                      <Pencil className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-loss hover:text-loss hover:bg-loss/10" onClick={() => setDeleteId(article.id)}>
